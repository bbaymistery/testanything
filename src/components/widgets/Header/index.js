      ...
      <nav className={styles.nav}>
        ...
        <div className={`${styles.ulDiv} ${styles.ulDiv_desktop}`}>
          <ul id="nav" className={`${styles.ul} `} >
            ...
            <i onClick={openOrCloseDropDown} className={` ${styles.fa_icon} ${animationDrop && styles.rotatedIcon} `}>
              <Image src={"/icons/angle-down.svg"} alt="Down arrow" width={16} height={16} />
            </i>
            {headerLinkNames?.map((item, i) => {
              return (
                <li key={item.id} className={`${styles.li_item} ${item.linkUrl === router.pathname && styles.active} ${i === 1 ? styles.hoverlayItem : ""}`} >
                  ...
                  {/* burda idi faicon */}
                  {i === 1 &&
                    <i onClick={openOrCloseDropDown} className={` ${styles.fa_icon_for_desktop}   ${animationDrop && styles.rotatedIcon}  ${activeLinkSelected === "heathrow transfer deals" ? "text_white" : ""}`}>
                      <Image src={"/icons/angle-down.svg"} alt="Down arrow" width={16} height={16} />
                    </i>
                  }
        ...
        <div className={`${styles.ulDiv}  ${styles.ulDiv_mobile}`}>
          <ul id="nav" className={`${styles.ul} ${activeClass ? styles.ulActive : "false"}`} style={{ borderBottom: `${activeClass ? "2px solid #ed8323" : "none"}`, }} ref={navRef}   >
            {/* position absolute olan sagda bakground oran olan icon */}
            {activeClass ? <i onClick={openOrCloseDropDown} className={` ${styles.fa_icon} ${animationDrop && styles.rotatedIcon} `}>
              <Image src={"/icons/angle-down.svg"} alt="Down arrow" width={16} height={16} />
            </i> : <></>}
            {
              activeClass ?
